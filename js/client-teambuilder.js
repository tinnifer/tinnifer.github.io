(function (exports, $) {
	...
	exports.TeambuilderRoom = exports.Room.extend({
		...
		renderSet: function (set, i) {
			...
			buf += '<label>Nickname</label><input type="text" name="nickname" class="textbox" value="' + BattleLog.escapeHTML(set.name || '') + '" placeholder="' + BattleLog.escapeHTML(species.fusionName || species.baseSpecies) + '" />';
			buf += '</div>';
		...
		renderTeambar: function () {
			...
			for (var i = start; i < end; i++) {
				...
				var sp = this.curTeam.dex.species.get(set.species);
				if (!set.species) {
					...
					buf += '<button disabled="disabled" class="pokemon">' + pokemonicon + BattleLog.escapeHTML(set.name || sp.fusionName || sp.baseSpecies || '<i class="fa fa-plus"></i>') + '</button> ';
				} else {
					buf += '<button name="selectPokemon" value="' + i + '" class="pokemon">' + pokemonicon + BattleLog.escapeHTML(set.name || sp.fusionName || sp.baseSpecies) + '</button> ';
				}
			}
		...
		chartChange: function (e, selectNext) {
			...
			switch (name) {
			case 'pokemon':
				var [head, body] = decodeFusionID(id);
				if (head in BattlePokedex || body && body in BattlePokedex)
				{
					val = this.curTeam.dex.species.get(e.currentTarget.value).name;
				}
				break;
			case 'ability':
