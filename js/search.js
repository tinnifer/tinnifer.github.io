...
(function (exports, $) {
	...
	Search.prototype.renderPokemonRow = function (pokemon, matchStart, matchLength, errorMessage, attrs) {						
		...
		var resultingPokemon = pokemon;
		if (this.engine.query.includes('/'))
		{
			var [head] = this.engine.query.split('/', 2);			
			pokemonName = Dex.species.get(head).name + '/' + pokemonName;			
			resultingPokemon = Dex.species.get(pokemonName);
		}
		
		if (Search.urlRoot) attrs += ' href="' + Search.urlRoot + 'pokemon/' + encodeURIComponent(id) + '" data-target="push"';
		var buf = '<li class="result"><a' + attrs + ' data-entry="pokemon|' + BattleLog.escapeHTML(pokemonName) + '">';
		...
		if (resultingPokemon.isFusion && resultingPokemon.exists)
		{
			var [head, body] = toID(resultingPokemon.baseSpecies).split('/');
			
			if (body && BattleFusionData[head] && BattleFusionData[head].customsprites[body])
			{
				name += '&sup1;';
			}
			
			if (body && BattleFusionData[body] && BattleFusionData[body].customsprites[head])
			{
				name += '&sup2;';
			}
		}
		...
		var types = resultingPokemon.types;
		...
		// base stats
		var stats = resultingPokemon.baseStats;
	...
	Search.prototype.renderTaggedPokemonRowInner = function (pokemon, tag, errorMessage) {
		...
		*/
		...
		buf += '<span class="col abilitycol">'; // was in the else block
		...
		for (var i in pokemon.abilities) {
			...
			buf += '<span class="col abilitycol">' + buf + '</span>';
		}
