...
find=function find(query){var _this$typedSearch;
query=toID(query);
if(this.query===query&&this.results){
return false;
}
this.query=query;
if(!query||((_this$typedSearch=this.typedSearch)==null?void 0:_this$typedSearch.searchType)==='pokemon'&&query.indexOf('/')===query.length-1){var _this$typedSearch2;
this.results=((_this$typedSearch2=this.typedSearch)==null?void 0:_this$typedSearch2.getResults(this.filters,this.sortCol,this.reverseSort))||[];
}else{
...
setType=function setType(searchType){var _this$typedSearch3;var format=arguments.length>1&&arguments[1]!==undefined?arguments[1]:'';var speciesOrSet=arguments.length>2&&arguments[2]!==undefined?arguments[2]:'';

this.results=null;

if(searchType!==((_this$typedSearch3=this.typedSearch)==null?void 0:_this$typedSearch3.searchType)){
this.filters=null;
this.sortCol=null;
}
...
illegalLabel=function illegalLabel(id){var _this$typedSearch4,_this$typedSearch4$il;
return((_this$typedSearch4=this.typedSearch)==null?void 0:(_this$typedSearch4$il=_this$typedSearch4.illegalReasons)==null?void 0:_this$typedSearch4$il[id])||null;
};_proto.

getTier=function getTier(species){var _this$typedSearch5;
return((_this$typedSearch5=this.typedSearch)==null?void 0:_this$typedSearch5.getTier(species))||'';
};_proto.

textSearch=function textSearch(query){var _this$typedSearch6,_this$typedSearch7;
query=toID(query);
...
var _query$split=query.split('/'),head=_query$split[0],body=_query$split[1];
query=body||'';
}
...
var _decodeFusionID=decodeFusionID(speciesid),first=_decodeFusionID[0],second=_decodeFusionID[1];
if(second){
...
var _decodeFusionID2=decodeFusionID(speciesid),first=_decodeFusionID2[0],second=_decodeFusionID2[1];
...
this.formatType==='fusion'?"fusion":"gen"+
gen;
...
{
var ability=species.abilities[i];
if(!ability)continue;
...
var abilities=[];
for(var _i11 in this.getTable()){
var _ability=dex.abilities.get(_i11);
if(_ability.isNonstandard)continue;
if(_ability.gen>dex.gen)continue;
abilities.push(_ability.id);
}

var goodAbilities=[['header',"Abilities"]];
var poorAbilities=[['header',"Situational Abilities"]];
var badAbilities=[['header',"Unviable Abilities"]];for(var _i13=0,_abilities$sort$map2=
abilities.sort().map(function(abil){return dex.abilities.get(abil);});_i13<_abilities$sort$map2.length;_i13++){var _ability2=_abilities$sort$map2[_i13];
var rating=_ability2.rating;
if(_ability2.id==='normalize')rating=3;
if(rating>=3){
goodAbilities.push(['ability',_ability2.id]);
}else if(rating>=2){
poorAbilities.push(['ability',_ability2.id]);
}else{
badAbilities.push(['ability',_ability2.id]);
}
}
...
var ability=this.dex.abilities.get(row[1]);for(var _i15=0;_i15<
filters.length;_i15++){var _ref14=filters[_i15];var filterType=_ref14[0];var value=_ref14[1];
switch(filterType){
case'pokemon':
...
var speciesSpecific=[];for(var _i17=0;_i17<
results.length;_i17++){var _this$dex$items$get$i;var row=results[_i17];
...
if(!filters)return true;
if(row[0]!=='ability')return true;
var ability=this.dex.abilities.get(row[1]);for(var _i19=0;_i19<
filters.length;_i19++){var _ref15=filters[_i19];var filterType=_ref15[0];var value=_ref15[1];
switch(filterType){
case'pokemon':
if(!Dex.hasAbility(this.dex.species.get(value),ability.name))return false;
break;
...
if(!excludedForme(species))speciesTypes.push.apply(speciesTypes,baseSpecies.types);for(var _i21=0,_baseSpecies$otherFor2=
baseSpecies.otherFormes;_i21<_baseSpecies$otherFor2.length;_i21++){var formeName=_baseSpecies$otherFor2[_i21];
...
var valid=false;for(var _i23=0;_i23<
moveTypes.length;_i23++){var type=moveTypes[_i23];
...
var usableMoves=[];
var uselessMoves=[];for(var _i25=0,_moves2=
moves;_i25<_moves2.length;_i25++){var _id12=_moves2[_i25];
...
sketchMoves.length;_i27++){var _id13=sketchMoves[_i27];
...
var move=this.dex.moves.get(row[1]);for(var _i29=0;_i29<
filters.length;_i29++){var _ref16=filters[_i29];var filterType=_ref16[0];var value=_ref16[1];
switch(filterType){
case'type':
